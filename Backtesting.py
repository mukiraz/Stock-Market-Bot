...
for i in range(len(candles)):
    if parameters["has_cash"]:
        if candles["Decision"][i] == "Long":
            ...
            parameters["coin_amount"] = calculate_coin_amount(parameters["position"], parameters["buy_price"], parameters["stop_price"], parameters["cash"], parameters["risk_percentage"])
            ...
            parameters["cash"] -= fee
            toggle_has()
            parameters["total_cash"] = parameters["cash"] + fee - parameters["comission"] * parameters["buy_price"]
            parameters["operation_time"] = candles["Id"][i+1]
            data = insert_transaction(parameters["operation_time"], "Buy", parameters["position"], 
                               ...
                               fee, parameters["total_cash"], parameters["stop_price"], parameters["sell_price"]
                               )
    ...
    if parameters["has_coin"]:
        if parameters["position"] == "Long":
            if  candles["High"][i] >= parameters["sell_price"]:
                ...
                parameters["cash"] += fee
                parameters["total_cash"] = parameters["cash"] - parameters["comission"] * parameters["sell_price"]
                ...
                parameters["cash"] += fee
                parameters["total_cash"] = parameters["cash"] - parameters["comission"] * parameters["stop_price"]
